    ...
    TCtx: 'static,
{
    ...
    pub fn export_ts<TPath: AsRef<Path>>(
        &self,
        export_path: TPath,
        bigint: Option<ts::BigIntExportBehavior>,
    ) -> Result<(), ExportError> {
        ...
        let config = if let Some(bigint) = bigint {
            ExportConfiguration::new().bigint(bigint)
        } else {
            ExportConfiguration::new().bigint(ts::BigIntExportBehavior::FailWithReason(
